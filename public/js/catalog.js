 ...
 *   - Manejar interacciones de UI (filtros, paginación, vistas)
 *   - Enviar las peticiones con los parámetros correctos al backend
 * 
 * Toda la lógica de negocio (parseBrand, parseSpecs, filtrado, paginación, ordenamiento)
 * ahora vive en el backend: src/services/productService.js
 */

// Variables de estado (solo UI)
let selectedBrands = [];
let minPrice = 0;
let maxPrice = 3000;
let minPriceLimit = 0;
...
// Elementos del Slider de Precio
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Si hay parámetro de búsqueda en la URL, usarlo
...
// ============================================================
// 1. FETCH DE DATOS DESDE EL API
// ============================================================

/**
 * Obtiene los productos del backend con filtros server-side.
 */
...
/**
 * Obtiene el rango de precios del catálogo.
 */
...
/**
 * Obtiene las marcas con conteo desde el backend.
 */
...
// ============================================================
// 2. RENDERIZADO (SOLO PRESENTACIÓN)
// ============================================================

/**
 * Renderiza las tarjetas de producto ya enriquecidas por el backend.
 * @param {Array} products - Productos enriquecidos del API
 */
function renderCatalogPage(products) {
  ...
  productsDisplayGrid.innerHTML = products.map(p => {
    // Los datos ya vienen enriquecidos del backend
    const formattedPrice = p.pricing.priceFormatted;
    const dateOnly = p.dateOnly;
    const brand = p.brand;
    const specs = p.specs;

    const imageElement = `<img src="${p.imagen_url || laptopPlaceholderBase64}" alt="${p.nombre} - Tienda Gamer EC" loading="lazy" onerror="this.src='${laptopPlaceholderBase64}';this.onerror=null;">`;
      ...
      return `
        <div class="grid-card">
          <div class="card-image-wrapper">
            ...
            <a href="/producto/laptops/${p.sku}">${imageElement}</a>
          </div>
          <div class="card-sku">${p.sku}</div>
          <h3 class="card-title" title="${p.nombre}">
            <a href="/producto/laptops/${p.sku}">${p.nombre}</a>
          </h3>
...
/**
 * Renderiza los checkboxes de marca con datos del backend.
 * @param {Array<{ brand: string, count: number }>} brands 
 */
 ...
 * @param {number} totalPages - Total de páginas del API
 */
...
function setupPriceSlider() {
  ...
  // Lógica de arrastre de Thumbs en el slider
...
function setupPaginationControls() {
  // changePage se expone globalmente arriba
}
