...
function renderCatalogPage(products) {
  ...
  productsDisplayGrid.innerHTML = products.map(p => {
    ...
    const imageElement = `<img src="${p.imagen_url || laptopPlaceholderBase64}" alt="${p.nombre}" loading="lazy" onerror="this.src='${laptopPlaceholderBase64}';this.onerror=null;">`;
      ...
      return `
        <div class="grid-card">
          <div class="card-image-wrapper">
            ...
            <a href="/product.html?sku=${p.sku}">${imageElement}</a>
          </div>
          <div class="card-sku">${p.sku}</div>
          <h3 class="card-title" title="${p.nombre}">
            <a href="/product.html?sku=${p.sku}">${p.nombre}</a>
          </h3>
